...
class TestTheoryGuidedModelController(BaseTestCase):
    ...
    def test_theory_guidedmodels_id_delete(self):
        ...
        response = self.client.open(
            '/v1.5.0/theory-guidedmodels/{id}'.format(id='id_example', user='user_example'),
            method='DELETE',
            headers=headers)
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
    ...
    def test_theory_guidedmodels_id_get(self):
        ...
        response = self.client.open(
            '/v1.5.0/theory-guidedmodels/{id}'.format(id='id_example'),
            method='GET',
            headers=headers,
            query_string=query_string)
        self.assert200(response,
    ...
    def test_theory_guidedmodels_post(self):
        ...
        response = self.client.open(
            '/v1.5.0/theory-guidedmodels'.format(user='user_example'),
            method='POST',
            headers=headers,
            data=json.dumps(theory_guided_model),
            content_type='application/json')
