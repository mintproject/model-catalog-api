...
class TestCausalDiagramController(BaseTestCase):
    ...
    def test_causaldiagrams_id_delete(self):
        ...
        response = self.client.open(
            '/v1.5.0/causaldiagrams/{id}'.format(id='id_example', user='user_example'),
            method='DELETE',
            headers=headers)
        self.assert200(response,
                       'Response body is : ' + response.data.decode('utf-8'))
    ...
    def test_causaldiagrams_id_get(self):
        ...
        response = self.client.open(
            '/v1.5.0/causaldiagrams/{id}'.format(id='id_example'),
            method='GET',
            headers=headers,
            query_string=query_string)
        self.assert200(response,
  ...
  "type" : [ "type", "type" ]
        ...
        response = self.client.open(
            '/v1.5.0/causaldiagrams'.format(user='user_example'),
            method='POST',
            headers=headers,
            data=json.dumps(causal_diagram),
            content_type='application/json')
